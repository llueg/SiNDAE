"""
fedbatch.py — Fed-batch bioreactor UDE example.

Demonstrates SiNDAE on a four-state ODE with Monod growth kinetics.  The NN
replaces the specific growth rate μ(S) from noisy state observations.
...
plot_folder = os.path.join(os.path.dirname(__file__), 'plots', 'fedbatch')
os.makedirs(plot_folder, exist_ok=True)

# ── Problem and MLP ───────────────────────────────────────────────────────────

problem = FedBatchBioreactorProblem(nfe=NFE_DATA, ncp=NCP_DATA)
