...
```python
generate_data(problem: ProblemDefinition, obs_every: int = 1, seed: int = 0, noise_std: Optional[np.ndarray] = None, pounce_options: Optional[dict] = None, tee: bool = False) -> InstanceData
```
...
- **`pounce_options`** (`Optional[dict]`, default `None`) — Extra POUNCE solver options, e.g. {'tol': 1e-9}.
- **`tee`** (`bool`, default `False`) — Pass through to the POUNCE solver (print output if True).
